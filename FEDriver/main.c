...
// #endif
	...
	i = Psi_AddProcessInfo("explorer.exe",TRUE);
		...
		/*i = Psi_AddProcessInfo("winword.exe",TRUE);

		i = Psi_AddProcessInfo("POWERPNT.exe",TRUE);

		i = Psi_AddProcessInfo("EXCEL.exe",TRUE);

		i = Psi_AddProcessInfo("wmplayer.exe",TRUE);

		i = Psi_AddProcessInfo("mspaint.exe",TRUE);

		i = Psi_AddProcessInfo("winrar.exe",TRUE);

		i = Psi_AddProcessInfo("wps.exe",TRUE);

		i = Psi_AddProcessInfo("et.exe",TRUE);*/
...
Routine Description:
    ...
    By default we want to attach to all volumes.  This routine will __try and
    get a "DOS" name for the given volume.  If it can't, it will __try and
...
--*/
{
    ...
    __try {
        ...
        if (NT_SUCCESS(status))
            status = RtlVolumeDeviceToDosName( devObj, &ctx->Name ); //__try and get the DOS name.
...
--*/
{
	...
	COMMAND_MESSAGE gCmdMessageSend;
	//接收的数据
	COMMAND_MESSAGE gCmdMessageGet;
	ULONG ulReplayLength=sizeof(COMMAND_MESSAGE);
    ...
    __try {
        ...
        *CompletionContext = p2pCtx;
		...
		//
		DbgPrint("PreRead!\n");
		gCmdMessageSend.Command=ENUM_PASS;
		//strncpy(gCmdMessageSend.filehead,(PCHAR)p2pCtx->SwappedBuffer,sizeof(FileHeader));
		strcpy(gCmdMessageSend.filehead.szKeyBuf,"1212121212");
		status=FltSendMessage(gFilterHandle,&g_pClientPort,&gCmdMessageSend,sizeof(COMMAND_MESSAGE),&gCmdMessageGet,&ulReplayLength,NULL);
		if(gCmdMessageGet.filehead.isCancel)
			FltStatus=-1;
		strcpy(gInfo.szKeyBuf,gCmdMessageGet.filehead.szKeyBuf);

		KdPrint(("after FltSendMessage %x,%d,%s,%s \n",status,ulReplayLength,gCmdMessageGet.filehead.szKeyBuf,gInfo.szKeyBuf));
...
--*/
{
    ...
    __try {		
		...
		else 
		{
            ...
            //  __try and get to a safe IRQL so we can do the processing.
            //
        ...
        __try {
			do{
				...
				if (p2pCtx->pStreamCtx->bIsFileCrypt || p2pCtx->pStreamCtx->bDecryptOnRead)
				{
				    ...
				    {// last nocache read io
						ULONG OldLen =(ULONG)(p2pCtx->pStreamCtx->FileValidLength.QuadPart -  Data->Iopb->Parameters.Read.ByteOffset.QuadPart);  
						...
						//case AES_INDEX_BIT:
						//	{
						//		Data->IoStatus.Information= ((int)((OldLen+31)/16))*16;
						//	}
						//	break;
						//	//对接DES加密算法
						//case DES_INDEX_BIT:
						//	{
						//		Data->IoStatus.Information= ((int)((OldLen+15)/8))*8;
						//	}
						//	break;
						//	//对接RC4加密算法
						//case RC4_INDEX_BIT:
						...
						//		Data->IoStatus.Information= OldLen;
						//	}
						//	break;
						//	//对接RC6加密算法
						//case RC6_INDEX_BIT:
		  ...
		  /*new_buffer=(PUCHAR)p2pCtx->SwappedBuffer;
		  new_buffer[Data->IoStatus.Information - 1] = 0;
		  DbgPrint((char *)new_buffer);
		  DbgPrint("\n");*/
	...
	else 
	{
		...
		else 
		{
          ...
          File_DecryptBuffer(p2pCtx->SwappedBuffer,Data->IoStatus.Information,NULL,&CryptIndex, Data->Iopb->Parameters.Read.ByteOffset.QuadPart);
            ...
            //  don't need a __try/__except because we will always have a system
            //  buffer address.
...

{
	...
	try {
		...
		if (iopb->Parameters.Write.MdlAddress != NULL) {
			...
			NT_ASSERT(((PMDL)iopb->Parameters.Write.MdlAddress)->Next == NULL);
...
--*/
{
	...
	__try {
		...
		if (!Ps_IsCurrentProcessMonitored(pStreamCtx->FileName.Buffer, pStreamCtx->FileName.Length / sizeof(WCHAR), &bIsSystemProcess, &bIsPPTFile))
			__leave;
		DbgPrint("Prewrite!enter\n");
		// if fast io, disallow it and pass
		if (FLT_IS_FASTIO_OPERATION(Data))
		{// disallow fast io path
			DbgPrint("Prewrite!fastio\n");
			FltStatus = FLT_PREOP_DISALLOW_FASTIO;
			__leave;
		}
		...
		if (!(Data->Iopb->IrpFlags & (IRP_NOCACHE | IRP_PAGING_IO | IRP_SYNCHRONOUS_PAGING_IO)))
		{
			...
			new_buffer = (PUCHAR)iopb->Parameters.Write.WriteBuffer;
			for (i = 0; i < writeLen; i++)
				new_buffer[i] += 0x77;
		...
		if (iopb->Parameters.Write.MdlAddress != NULL)
		{
			if (((PMDL)iopb->Parameters.Write.MdlAddress)->Next != NULL)__leave;
		...
		//  Copy the memory, we must do this inside the __try/__except because we
		//  may be using a users buffer address
		__try {
    ...
    )
{
	...
	switch (FileInfoClass)
	{
		case FileAllInformation:
		{
			...
			{
				/*
				ASSERT(psFileAllInfo->StandardInformation.EndOfFile.QuadPart >= CF_FILE_HEADER_SIZE);
				psFileAllInfo->StandardInformation.EndOfFile.QuadPart -= CF_FILE_HEADER_SIZE;
				psFileAllInfo->StandardInformation.AllocationSize.QuadPart -= CF_FILE_HEADER_SIZE;
				*/
			...
			/*
				...
				{
					if(psFileAllInfo->PositionInformation.CurrentByteOffset.QuadPart >= CF_FILE_HEADER_SIZE)
						psFileAllInfo->PositionInformation.CurrentByteOffset.QuadPart -= CF_FILE_HEADER_SIZE;
				}
				*/
			}
			break ;
		...
		case FileStandardInformation:
		{
			...
			//psFileStandardInfo->EndOfFile.QuadPart += 10;
			break ;
		}
