...
//
//  __leave space for additional FS_FILTER codes here
//
...
typedef union _FLT_PARAMETERS {
    ...
    union {
        ...
        struct {
            ...
            //  probed & captured before access, or use __try-__except to enclose
...
//
//  The filter should never free or __try to replace the buffer in the Name
...
#define FLT_VALID_FILE_NAME_QUERY_METHODS 0x0000ff00
    ...
    //  the Filter Manager __try to retrieve the name from the cache first, and,
    ...
    //
    //  Query the Filter Manager's name cache for the name, but don't __try
    ...
    //  found __try to query the file system if it is safe to do so.
    //
...
//  been set, it can not be freed __except in the free context callback
//  routine.
//
...
//  KeExitCriticalRegion to disable APCs (__except Special Kernel APCs) while
//  the lock is held
//
