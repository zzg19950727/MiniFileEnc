...
ULONG Psi_AddProcessInfo(PUCHAR pszProcessName, BOOLEAN bMonitor)
{
	...
	__try{
		...
		*/
		...
		else if (!_strnicmp(pszProcessName, "mspaint.exe", strlen("mspaint.exe")))
		{
			wcscpy(psProcInfo->wsszRelatedExt[0],  L".bmp") ;
			wcscpy(psProcInfo->wsszRelatedExt[1],  L".jpg") ;
			wcscpy(psProcInfo->wsszRelatedExt[2],  L".jpeg") ;
			wcscpy(psProcInfo->wsszRelatedExt[3],  L".png") ;
			wcscpy(psProcInfo->wsszRelatedExt[4], L".tif") ;
			wcscpy(psProcInfo->wsszRelatedExt[5],  L".tiff") ;
			wcscpy(psProcInfo->wsszRelatedExt[6], L".gif") ;
		}
		else if (!_strnicmp(pszProcessName, "winrar.exe", strlen("winrar.exe")))
		{
			wcscpy(psProcInfo->wsszRelatedExt[0],  L".rar") ;
			wcscpy(psProcInfo->wsszRelatedExt[1],  L".zip") ;
		}
		else if (!_strnicmp(pszProcessName, "wps.exe", strlen("wpsnotify.exe")))
		{
